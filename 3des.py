...
def encrypt(pt, key):
    ...
    rk = []
    for i in range(0, 16):
        leftk = shift_left(leftk, shift_table[i])
        rightk = shift_left(rightk, shift_table[i])
        
        combine_str = leftk + rightk
...
def decrypt(ct, key):
    ...
    for i in range(0, 16):
        ...
        if(i != 15):
            left, right = right, left
        print("Round ", i + 1, " ", bin2hex(left), " ", bin2hex(right), " ", rk_rev[i])
...
key1 = permute(key1, keyp, 56)
key2 = permute(key2, keyp, 56)
key3 = permute(key3, keyp, 56)

print("Encryption:")
cipher_text1 = bin2hex(encrypt(pt, key1))
cipher_text2 = bin2hex(decrypt(cipher_text1, key2))
cipher_text = bin2hex(encrypt(cipher_text2, key3))
print("Cipher Text obtained: ",cipher_text)

print("Decryption:")
text1 = bin2hex(decrypt(cipher_text, key3))
text2 = bin2hex(encrypt(text1, key2))
text = bin2hex(decrypt(text2, key1))
